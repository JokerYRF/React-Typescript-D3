...
export default class DagreGraph extends Component<GraphProps> {
  ...
  _drawChart = () => {
    ...
    //initialScale元素放大倍数，随着父元素宽高发生变化时改变初始渲染大小
    let initialScale = max / 779;
    var tWidth =
      (svg._groups[0][0].clientWidth - g.graph().width * initialScale) / 2;
    var tHeight =
      (svg._groups[0][0].clientHeight - g.graph().height * initialScale) / 2;
    svg.call(
      zoom.transform,
